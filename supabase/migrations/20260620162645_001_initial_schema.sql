/*
# BIRD Strategic Planning - Initial Schema

1. Purpose
Comprehensive database schema for the BIRD (Bangsamoro Investment Roadmap Development)
AI-powered strategic planning PWA. Supports user authentication, strategic plans, SWOT analysis,
strategy matrix, balanced scorecard, PAPs management, team collaboration, activity logging,
plan templates, notifications, and admin functionality.

2. New Tables
- users (extends auth.users): profiles, org details, roles, notification preferences
- organizations: teams/groups with metadata
- organization_members: team membership with roles
- strategic_plans: top-level strategic plans with sync metadata
- plan_templates: reusable templates including pre-built industry templates
- plan_shares: sharing records with permission levels
- swot_items: SWOT analysis entries per plan
- strategy_matrix: TOWS strategy entries
- balanced_scorecards: BSC perspectives per plan
- scorecard_objectives: objectives within scorecards
- scorecard_kpis: KPIs linked to objectives
- paps: Programs, Activities, Projects tracking
- activity_logs: comprehensive audit trail of all changes
- comments: threaded comments on plan elements
- plan_collaboration: presence tracking for real-time collaboration
- user_notifications: notification queue and preferences

3. Security
- RLS enabled on ALL tables
- Owner-scoped policies for personal data
- Team-scoped policies for shared data
- Admin-only policies for user management

4. Indexes
- user_id indexes for fast lookups
- plan_id indexes for plan-scoped queries
...
-- Users: self access, admin all access
...
CREATE POLICY "users_select_admin" ON users FOR SELECT
  TO authenticated USING (EXISTS (SELECT 1 FROM users u WHERE u.id = auth.uid() AND u.role IN ('admin', 'super_admin')));
...
-- Organizations: members can access, creator can update
...
CREATE POLICY "org_insert_creator" ON organizations FOR INSERT
  TO authenticated WITH CHECK (created_by = auth.uid());

DROP POLICY IF EXISTS "org_update_creator" ON organizations;
CREATE POLICY "org_update_creator" ON organizations FOR UPDATE
...
CREATE POLICY "org_members_insert_admin" ON organization_members FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = organization_members.organization_id AND om.user_id = auth.uid() AND om.role = 'admin')
  );

DROP POLICY IF EXISTS "org_members_update_admin" ON organization_members;
CREATE POLICY "org_members_update_admin" ON organization_members FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = organization_members.organization_id AND om.user_id = auth.uid() AND om.role = 'admin')
  );

DROP POLICY IF EXISTS "org_members_delete_admin" ON organization_members;
CREATE POLICY "org_members_delete_admin" ON organization_members FOR DELETE
  TO authenticated USING (
    user_id = auth.uid() OR
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = organization_members.organization_id AND om.user_id = auth.uid() AND om.role = 'admin')
  );

-- Strategic plans: owner or shared access
DROP POLICY IF EXISTS "plans_select_owner_or_shared" ON strategic_plans;
CREATE POLICY "plans_select_owner_or_shared" ON strategic_plans FOR SELECT
  TO authenticated USING (
    user_id = auth.uid() OR
    organization_id IN (SELECT organization_id FROM organization_members WHERE user_id = auth.uid()) OR
    id IN (SELECT plan_id FROM plan_shares WHERE (shared_with_user_id = auth.uid() OR shared_with_org_id IN (SELECT organization_id FROM organization_members WHERE user_id = auth.uid())) AND accepted = true)
  );

DROP POLICY IF EXISTS "plans_insert_owner" ON strategic_plans;
CREATE POLICY "plans_insert_owner" ON strategic_plans FOR INSERT
  TO authenticated WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "plans_update_owner_or_editor" ON strategic_plans;
CREATE POLICY "plans_update_owner_or_editor" ON strategic_plans FOR UPDATE
  TO authenticated USING (
    user_id = auth.uid() OR
    id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin')) OR
    organization_id IN (SELECT organization_id FROM organization_members WHERE user_id = auth.uid() AND role IN ('admin', 'editor'))
  );

DROP POLICY IF EXISTS "plans_delete_owner" ON strategic_plans;
CREATE POLICY "plans_delete_owner" ON strategic_plans FOR DELETE
  TO authenticated USING (user_id = auth.uid());

-- SWOT items
DROP POLICY IF EXISTS "swot_select_plan_access" ON swot_items;
CREATE POLICY "swot_select_plan_access" ON swot_items FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = swot_items.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );

DROP POLICY IF EXISTS "swot_insert_plan_access" ON swot_items;
CREATE POLICY "swot_insert_plan_access" ON swot_items FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = swot_items.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
    ))
  );

DROP POLICY IF EXISTS "swot_update_plan_access" ON swot_items;
CREATE POLICY "swot_update_plan_access" ON swot_items FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = swot_items.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "swot_delete_plan_access" ON swot_items FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = swot_items.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
    ))
  );

-- Strategy matrix
DROP POLICY IF EXISTS "strategy_select_plan_access" ON strategy_matrix;
CREATE POLICY "strategy_select_plan_access" ON strategy_matrix FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = strategy_matrix.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "strategy_insert_plan_access" ON strategy_matrix FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = strategy_matrix.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "strategy_update_plan_access" ON strategy_matrix FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = strategy_matrix.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "strategy_delete_plan_access" ON strategy_matrix FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = strategy_matrix.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE POLICY "bsc_select_plan_access" ON balanced_scorecards FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = balanced_scorecards.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "bsc_insert_plan_access" ON balanced_scorecards FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = balanced_scorecards.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "bsc_update_plan_access" ON balanced_scorecards FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = balanced_scorecards.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "bsc_delete_plan_access" ON balanced_scorecards FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = balanced_scorecards.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE POLICY "obj_select_plan_access" ON scorecard_objectives FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_objectives.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "obj_insert_plan_access" ON scorecard_objectives FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_objectives.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "obj_update_plan_access" ON scorecard_objectives FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_objectives.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "obj_delete_plan_access" ON scorecard_objectives FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_objectives.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE POLICY "kpi_select_plan_access" ON scorecard_kpis FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_kpis.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "kpi_insert_plan_access" ON scorecard_kpis FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_kpis.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "kpi_update_plan_access" ON scorecard_kpis FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_kpis.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "kpi_delete_plan_access" ON scorecard_kpis FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = scorecard_kpis.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE POLICY "paps_select_plan_access" ON paps FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = paps.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "paps_insert_plan_access" ON paps FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = paps.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "paps_update_plan_access" ON paps FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = paps.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "paps_delete_plan_access" ON paps FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = paps.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE POLICY "activity_select_plan_access" ON activity_logs FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = activity_logs.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "activity_insert_plan_access" ON activity_logs FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = activity_logs.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
...
CREATE POLICY "comments_select_plan_access" ON comments FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = comments.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "comments_insert_plan_access" ON comments FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = comments.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
...
CREATE POLICY "collab_select_plan_access" ON plan_collaboration FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_collaboration.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "collab_insert_plan_access" ON plan_collaboration FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_collaboration.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
...
CREATE POLICY "collab_update_own" ON plan_collaboration FOR UPDATE
  TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "collab_delete_own" ON plan_collaboration;
CREATE POLICY "collab_delete_own" ON plan_collaboration FOR DELETE
  TO authenticated USING (user_id = auth.uid());
...
CREATE POLICY "notifications_delete_own" ON user_notifications FOR DELETE
  TO authenticated USING (user_id = auth.uid());

-- Plan snapshots
DROP POLICY IF EXISTS "snapshots_select_plan_access" ON plan_snapshots;
CREATE POLICY "snapshots_select_plan_access" ON plan_snapshots FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_snapshots.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "snapshots_insert_plan_access" ON plan_snapshots FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_snapshots.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "templates_select_public" ON plan_templates FOR SELECT
  TO authenticated USING (is_public = true OR created_by = auth.uid() OR organization_id IN (SELECT organization_id FROM organization_members WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "templates_insert_owner" ON plan_templates;
CREATE POLICY "templates_insert_owner" ON plan_templates FOR INSERT
  TO authenticated WITH CHECK (created_by = auth.uid());

DROP POLICY IF EXISTS "templates_update_owner" ON plan_templates;
CREATE POLICY "templates_update_owner" ON plan_templates FOR UPDATE
  TO authenticated USING (created_by = auth.uid()) WITH CHECK (created_by = auth.uid());

DROP POLICY IF EXISTS "templates_delete_owner" ON plan_templates;
CREATE POLICY "templates_delete_owner" ON plan_templates FOR DELETE
  TO authenticated USING (created_by = auth.uid());

-- Plan shares
DROP POLICY IF EXISTS "shares_select_plan_owner" ON plan_shares;
CREATE POLICY "shares_select_plan_owner" ON plan_shares FOR SELECT
  TO authenticated USING (
    shared_by = auth.uid() OR
    shared_with_user_id = auth.uid() OR
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_shares.plan_id AND sp.user_id = auth.uid())
  );

DROP POLICY IF EXISTS "shares_insert_plan_owner" ON plan_shares;
CREATE POLICY "shares_insert_plan_owner" ON plan_shares FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_shares.plan_id AND sp.user_id = auth.uid())
  );

DROP POLICY IF EXISTS "shares_update_plan_owner" ON plan_shares;
CREATE POLICY "shares_update_plan_owner" ON plan_shares FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_shares.plan_id AND sp.user_id = auth.uid())
  );

DROP POLICY IF EXISTS "shares_delete_plan_owner" ON plan_shares;
CREATE POLICY "shares_delete_plan_owner" ON plan_shares FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = plan_shares.plan_id AND sp.user_id = auth.uid())
  );
...
CREATE POLICY "cld_select_plan_access" ON causal_loops FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = causal_loops.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "cld_insert_plan_access" ON causal_loops FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = causal_loops.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "cld_update_plan_access" ON causal_loops FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = causal_loops.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "cld_delete_plan_access" ON causal_loops FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = causal_loops.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE POLICY "archetype_select_plan_access" ON systems_archetypes FOR SELECT
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = systems_archetypes.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND accepted = true)
    ))
  );
...
CREATE POLICY "archetype_insert_plan_access" ON systems_archetypes FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = systems_archetypes.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "archetype_update_plan_access" ON systems_archetypes FOR UPDATE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = systems_archetypes.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level IN ('editor', 'admin'))
...
CREATE POLICY "archetype_delete_plan_access" ON systems_archetypes FOR DELETE
  TO authenticated USING (
    EXISTS (SELECT 1 FROM strategic_plans sp WHERE sp.id = systems_archetypes.plan_id AND (
      sp.user_id = auth.uid() OR
      sp.id IN (SELECT plan_id FROM plan_shares WHERE shared_with_user_id = auth.uid() AND permission_level = 'admin')
...
CREATE TRIGGER tr_orgs_updated_at BEFORE UPDATE ON organizations
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS tr_plans_updated_at ON strategic_plans;
CREATE TRIGGER tr_plans_updated_at BEFORE UPDATE ON strategic_plans
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS tr_swot_updated_at ON swot_items;
CREATE TRIGGER tr_swot_updated_at BEFORE UPDATE ON swot_items
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS tr_strategy_updated_at ON strategy_matrix;
CREATE TRIGGER tr_strategy_updated_at BEFORE UPDATE ON strategy_matrix
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS tr_obj_updated_at ON scorecard_objectives;
CREATE TRIGGER tr_obj_updated_at BEFORE UPDATE ON scorecard_objectives
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS tr_kpi_updated_at ON scorecard_kpis;
CREATE TRIGGER tr_kpi_updated_at BEFORE UPDATE ON scorecard_kpis
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

DROP TRIGGER IF EXISTS tr_paps_updated_at ON paps;
CREATE TRIGGER tr_paps_updated_at BEFORE UPDATE ON paps
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE TRIGGER tr_swot_activity AFTER INSERT OR UPDATE OR DELETE ON swot_items
  FOR EACH ROW EXECUTE FUNCTION log_activity();

DROP TRIGGER IF EXISTS tr_strategy_activity ON strategy_matrix;
CREATE TRIGGER tr_strategy_activity AFTER INSERT OR UPDATE OR DELETE ON strategy_matrix
  FOR EACH ROW EXECUTE FUNCTION log_activity();

DROP TRIGGER IF EXISTS tr_obj_activity ON scorecard_objectives;
CREATE TRIGGER tr_obj_activity AFTER INSERT OR UPDATE OR DELETE ON scorecard_objectives
  FOR EACH ROW EXECUTE FUNCTION log_activity();

DROP TRIGGER IF EXISTS tr_kpi_activity ON scorecard_kpis;
CREATE TRIGGER tr_kpi_activity AFTER INSERT OR UPDATE OR DELETE ON scorecard_kpis
  FOR EACH ROW EXECUTE FUNCTION log_activity();

DROP TRIGGER IF EXISTS tr_paps_activity ON paps;
CREATE TRIGGER tr_paps_activity AFTER INSERT OR UPDATE OR DELETE ON paps
  FOR EACH ROW EXECUTE FUNCTION log_activity();
...
CREATE TRIGGER tr_plan_progress_kpi AFTER INSERT OR UPDATE ON scorecard_kpis
  FOR EACH ROW EXECUTE FUNCTION update_plan_progress();

DROP TRIGGER IF EXISTS tr_plan_progress_paps ON paps;
CREATE TRIGGER tr_plan_progress_paps AFTER INSERT OR UPDATE ON paps
  FOR EACH ROW EXECUTE FUNCTION update_plan_progress();
...
CREATE TRIGGER tr_auth_users_insert AFTER INSERT ON auth.users
